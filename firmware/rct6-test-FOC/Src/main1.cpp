...
#include "stdio.h"
#include "string.h"
#include "../lib/simpleFOC-next/drivers/BLDCDriver3PWM.h"
#include "../lib/simpleFOC-next/BLDCMotor.hpp"
...
void DRV8301_init(void){
  ...
  //   HAL_GPIO_WritePin(cs2_GPIO_Port, cs2_Pin, GPIO_PIN_RESET);
  //   SPI_GPIO_Transmit(spi_read_2, spi_recv);
  //   HAL_GPIO_WritePin(cs2_GPIO_Port, cs1_Pin, GPIO_PIN_SET);
  // }
}
/* USER CODE END PTD */
...
int main(void)
{
  ...
  // BLDCDriver3PWM _test_PWM(&htim1, 0, 0, 0);
  // _test_PWM.init();

  // BLDCMotor _test_motor(7, 0.1, 0.1, 0.1);

  // BLDCDriver3PWM _test_driver(&htim1, 0, 0, 0);
  // _test_driver.init();
  
  // _test_motor.linkDriver(&_test_driver);
  ...
  while (1)
  {
    ...
    // // raw_angle = bsp_as5600GetRawAngle();

    // // angle_int = (int)angle ;
    // sprintf(angle_str, "%f\n", angle);
    // // sprintf(angle_str, "%d\n", raw_angle);

    // HAL_UART_Transmit(&huart4, (uint8_t*)angle_str, strlen(angle_str), 1000);
    // // HAL_UART_Transmit(&huart4, (uint8_t*)angle_str, strlen(angle_str), 1000);
    // HAL_Delay(100);
    DRV8301_init();
    ...
    //   float a = i / 10000.0 * _2PI;

    //   _test_motor.setPhaseVoltage(12, 0, a);
    //   // float a = i / 10000.0 * 12;
    //   // _test_PWM.setPwm(a,a,a);
    //   HAL_Delay(1);
    // }
