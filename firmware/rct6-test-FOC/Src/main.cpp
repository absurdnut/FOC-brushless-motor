...
void DRV8301_init(void){
  ...
  uint16_t spi_write_2 = 0x17C0; //写入2寄存器
...
int main(void)
{
  ...
  BLDCMotor _test_motor(7);

  BLDCDriver3PWM _test_driver(&htim8, 0, 0, 0);
  _test_driver.init();
  
  _test_motor.linkDriver(&_test_driver);
  // DRV8301_init(); //初始化DRV8301,目前调试旋转编码器,暂时停用
