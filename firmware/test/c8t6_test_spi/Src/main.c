...
/* Private define ------------------------------------------------------------*/
/* USER CODE BEGIN PD */
void SPI_GPIO_Transmit(uint16_t data, uint16_t* recv) {
...
int main(void)
{
  ...
  MX_GPIO_Init();
  /* USER CODE BEGIN 2 */
  uint16_t spi_trans_1 = 0x0200;
  uint16_t spi_trans_2 = 0x8000;
  uint16_t spi_trans_3 = 0x9000;
  uint16_t spi_trans_4 = 0x9700;

  // uint16_t spi_trans_1 = 0x9800;
  // uint16_t spi_trans_2 = 0x1f88;
  uint16_t spi_recv = 0x00;
  /* USER CODE END 2 */
  ...
  while (1)
  {
    SPI_GPIO_Transmit(spi_trans_4, &spi_recv);
    HAL_Delay(100);
    /* USER CODE END WHILE */
