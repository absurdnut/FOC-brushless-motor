...
void sendSerial(float value1, float value2, float value3) {

  int cache1 = (int)(value1 * 100);
  int cache2 = (int)(value2 * 100);
  int cache3 = (int)(value3 * 100);

  char buffer[50];
  // 使用 sprintf 将浮点数格式化为字符串
  // cache = 14;
  sprintf(buffer, "Ua:%dUb:%dUc:%d\n", cache1, cache2, cache3);
...
*/
void BLDCMotor::linkDriver(BLDCDriver3PWM* _driver) {
  driver = _driver;
}
