...
int CurrentSense::alignBLDCDriver(float voltage, BLDCDriver* bldc_driver, bool modulation_centered){
    ...
    float ca[3] = {static_cast<float>(fabs(c_a.a)), static_cast<float>(fabs(c_a.b)), static_cast<float>(fabs(c_a.c))};
    ...
    // and make sure it's around 2 (1.5 at least) times higher than the other two
    float cb[3] = {static_cast<float>(fabs(c_b.a)), static_cast<float>(fabs(c_b.b)), static_cast<float>(fabs(c_b.c))};
    max_i = -1; // max index
    max_c = 0; // max current
